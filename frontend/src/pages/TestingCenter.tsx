...
import {
  Play, Square, CheckCircle2, XCircle, Clock, Terminal,
  Cpu, RefreshCw, ChevronDown, ChevronUp, AlertTriangle,
} from 'lucide-react';
...
export const TestingCenter: React.FC = () => {
  ...
  useEffect(() => {
    apiService.getMeters().then(d => { setMeters(d); if (d.length) setSelectedMeter(d[0].id); }).catch(console.error);
    apiService.getTestSuites().then(d => { setSuites(d); if (d.length) setSelectedSuite(d[0].id); }).catch(console.error);
  }, []);
  ...
  const addLog = (msg: string) => setLogs(prev => [...prev, `[${new Date().toISOString().slice(11, 23)}] ${msg}`]);
  ...
  return (
    <div style={{ padding: '28px', display: 'flex', flexDirection: 'column', gap: '22px' }}>
      ...
      {/* ── Page Header ── */}
      <div>
        <h2 className="section-title">Automated DLMS Testing Center</h2>
        <p className="section-sub">Configure test suites, execute DLMS/COSEM association, and monitor real-time test execution.</p>
      </div>
