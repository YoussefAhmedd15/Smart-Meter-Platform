...
Write-Host "1. Starting FastAPI Backend on http://localhost:8000 ..." -ForegroundColor Green
Start-Process powershell -ArgumentList "-NoExit", "-Command", "python -m uvicorn backend.app.main:app --reload --port 8000"

Write-Host ""
Write-Host "2. Starting React Frontend Dashboard on http://localhost:3000 ..." -ForegroundColor Green
